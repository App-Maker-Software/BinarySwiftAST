...
public final class CommandLineTool {
    ...
    public static func proccess(_ tree1: SourceFileSyntax) throws -> String {
        var result = tree1.description
    ...
    public func run() throws {
        if let first = arguments.first {
            let folder = try Files.Folder(path: first)
            for swiftFile in folder.files.recursive.filter({
                $0.extension?.contains("swift") ?? false
            }) {
                let syntax = try SyntaxParser.parse(swiftFile.url)
                let swiftFileWithNewNamePath: String =  swiftFile.path.replacingOccurrences(of: swiftFile.name, with: "_generated_" + swiftFile.name)
                try Self.proccess(syntax).write(toFile: swiftFileWithNewNamePath, atomically: true, encoding: .utf8)
                print("rewrote: \(swiftFile.path)")
            }
        } else {
            print("you must pass a relative folder path as the first argument")
        }
    }
}
